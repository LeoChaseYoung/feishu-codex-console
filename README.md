...
- `会话` / `/sessions`：恢复或搜索绑定当前项目的历史会话、查看最近活动来源、压缩上下文，或在本机打开同一个 Codex thread。
...
项目工作台会把个人收藏放在最前、最近使用放在其后；同名项目始终同时显示路径。切换不会删除项目文件或 Codex 历史，但会停止当前飞书会话的未完成任务并解除旧 thread 绑定。新建 thread 会按第一条任务自动命名，之后可从“会话”中心恢复并与本机 Codex 接力。会话卡的“在本机打开当前会话”会打开同一个原生 thread，并标出最近活动来自飞书还是本机；运行或排队中会拒绝接力，防止两端同时写入。本机另开会话不会静默改变飞书绑定，飞书卡片和普通群消息也不会写入 Codex 历史。
