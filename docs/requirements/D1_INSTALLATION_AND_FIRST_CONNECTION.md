...
- 通过聊天收集 App Secret、验证码、恢复码或 Token。
...
> 作为不熟悉终端的体验者，我希望把一段受约束的提示词交给本机 Codex，由它代跑公开安装向导，并在官方登录、权限和发布步骤暂停让我确认。

## 6. 端到端安装流程

```text
手动运行，或让本机 Codex 按公开安全契约代跑
  ↓
...
安装助手不得从终端文字推测完成。它必须读取 `install-status --json` 和 `doctor --json`；本地 `ready/ok` 通过后，再由用户在飞书发送“状态”完成端到端确认。
...
| 安装助手失去上下文 | 返回持久化阶段和 `nextAction.code` | 只恢复下一安全步骤，不删除配置 |
| 登录/OTP 路径失败一次 | 明确停止并等待用户 | 不重复登录、不索要秘密、不切换认证猜测 |
...
| D1-FR-015 | 提供可复制的 Codex 辅助安装安全契约，并明确人机确认边界 | P0 |
| D1-FR-016 | 提供脱敏的 `install-status --json` 和 `doctor --json` 机器验收结果 | P0 |
...
- 用户无需手工编辑 `.env`。
- 安装结果可以通过 `install-status --json`、`doctor --json` 和飞书真实消息再次验证。
- Codex 辅助安装只在 `ready=true`、`ok=true` 且用户确认飞书回复后声明端到端完成。
...
| 能力 | 当前状态 | 需要补充 |
|---|---|---|
| Node/Codex/Bot 检查 | 已实现 | doctor 已提供脱敏 JSON；init 分步 UI 后续继续优化 |
| 三档权限预设 | 已实现 | 安装摘要和更清楚的影响说明 |
| 身份自动发现 | 已实现 | 确认页、重试和进度恢复 |
| 私有配置目录 | 已实现 | 原子写入、备份和未知键保留已完成 |
| doctor | 已实现 | 能力检查、`--fix` 与 `--json` |
| Codex 辅助安装 | 已实现 | 公开提示词、人工暂停点、机器状态和失败停止规则 |
...
- [x] D1-T17：增加 `install-status --json` 与 `doctor --json` 脱敏契约。
- [x] D1-T18：增加 Codex 辅助安装教程、SOP 映射和正逆向验收。
...
确认：复用唯一 CLI 向导，不建设第二套安装状态机。Codex 可以代跑检查和命令，但必须在账号、Secret、权限差异和应用发布处暂停；只按机器契约与飞书真实回复判定完成。
...
### 决策记录 2026-07-19 · D1-E

- 决策：公开“让 Codex 帮你安装”作为非开发者入口，底层仍调用 `npx feishu-codex-console@next init`。
- 机器契约：`install-status --json` 表达持久安装阶段、服务健康、两个事件消费者和下一步；`doctor --json` 表达脱敏检查结果。
- 人工边界：官方登录、App Secret、飞书权限差异、应用发布和最后一条飞书验证消息必须由用户完成。
- 停止规则：同一认证路径失败一次即停止，不重复 OTP、不索要秘密、不将本地就绪冒充端到端通过。
