...
- 在同一台设备、同一 Codex 账号与数据目录下，macOS 可以从会话卡直接打开这个原生 thread；不支持深链的平台必须显示可执行的 `codex resume <thread-id>` 降级命令，不能谎报已经打开。
...
- [x] D3-T10：实现“在 Codex 中打开”及同一原生 thread 的桌面端导航。
- [x] D3-T11：实现本机已有 thread 的搜索、项目一致性校验与显式绑定。
- [x] D3-T12：展示最近活动来源，并补齐双向接力、另开 thread 和失败降级测试。
