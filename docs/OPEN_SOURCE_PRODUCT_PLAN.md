...
多 Agent 保留为后期增强能力，不进入当前单 Agent 核心流程，也不作为首个公开版本的产品卖点。现有底层开关继续默认关闭，不删除实验代码。

重新启动该方向的前置条件：

- 单 Agent 的项目群、话题会话、任务判断和失败恢复已经稳定。
- 飞书与 Codex 桌面端可以接力同一个原生会话。
- 子任务能够完整继承项目、成员 ACL、沙箱和外部操作确认。
- 根任务与子 Agent 的审批、停止、恢复和 Token 统计形成统一闭环。
- 团队用户已经出现明确且可验证的并行开发需求。

后期产品形态采用“自动 / 单 Agent / 并行协作”三档。用户始终只管理一个任务和一个最终结果；子 Agent 的角色、状态和总用量收纳在同一任务卡内，不在群聊中输出多条内部过程消息。初期限制为最多 3 个子 Agent、递归深度 1，并保证同一工作区只有一个写 Agent。
